...
export function HeroBanner() {
  ...
  return (
    <div className="w-full relative group">
      {/* Main Container - Responsive Width with Header - Matches Grid/Col-Span-8 logic */}
      <div className="w-full max-w-5xl mx-auto relative px-4 md:px-0">
        ...
        <button
          onClick={prevSlide}
          className="hidden md:flex absolute -left-12 lg:-left-16 top-1/2 -translate-y-1/2 w-12 h-24 rounded bg-gradient-to-r from-slate-900/80 to-slate-900/40 hover:bg-slate-800 items-center justify-center text-white z-20 transition-all opacity-0 group-hover:opacity-100 hover:scale-105"
        >
          <ChevronLeft size={40} />
        </button>
        ...
        <button
          onClick={nextSlide}
          className="hidden md:flex absolute -right-12 lg:-right-16 top-1/2 -translate-y-1/2 w-12 h-24 rounded bg-gradient-to-l from-slate-900/80 to-slate-900/40 hover:bg-slate-800 items-center justify-center text-white z-20 transition-all opacity-0 group-hover:opacity-100 hover:scale-105"
        >
          <ChevronRight size={40} />
        </button>

        {/* Mobile Navigation Areas (Tap Left/Right) */}
        <div className="md:hidden absolute inset-y-0 left-0 w-12 z-20" onClick={prevSlide} />
        <div className="md:hidden absolute inset-y-0 right-0 w-12 z-20" onClick={nextSlide} />
        ...
        <div className="relative group/card transition-transform duration-300 hover:-translate-y-1">
          ...
          <div className="relative bg-[#0f1922] shadow-[0_35px_60px_-15px_rgba(0,0,0,0.6)] flex flex-col lg:grid lg:grid-cols-3 overflow-hidden rounded-md border border-white/5 ring-1 ring-white/10">
            {/* Left Side: Main Visual - Spans 2 cols */}
            <div className={`relative lg:col-span-2 aspect-video transition-all duration-500 ease-in-out bg-gradient-to-br ${activeBackground}`}>
              {/* Overlay Pattern */}
            ...
            {/* Right Side: Details & Screenshots - Spans 1 col */}
            <div className="w-full lg:col-span-1 bg-[#0a1016] p-6 flex flex-col justify-between relative z-10 box-border border-l border-white/5">
